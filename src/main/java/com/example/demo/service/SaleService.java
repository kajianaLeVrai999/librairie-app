...
public class SaleService {
    ...
    public List<GenderSalesDTO> getSalesByGenderBetweenDates(LocalDate startDate, LocalDate endDate) {
        return saleRepository.findAll().stream()
                ...
                .flatMap(s -> s.getBookCopy().getBook().getGenders().stream()
                        .map(g -> new Object[]{g, s}))
                        ...
                        Collectors.collectingAndThen(
                            ...
                            list -> {
                                ...
                                double revenue = list.stream()
                                    .mapToDouble(e -> ((Sale) e[1]).getTotalAmount())
                                    .sum();
                                long count = list.size();
                                return new GenderSalesDTO(
                                    gender.getId(),
                                    gender.getName(),
                                    count,
                                    revenue
